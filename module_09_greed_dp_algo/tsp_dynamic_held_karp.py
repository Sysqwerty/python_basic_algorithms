...
# Алгоритм Хелда-Карпа має час виконання O(2^n * 2^n), який є поліноміальним відносно кількості міст n, що є значним
# поліпшенням порівняно з факторіальним часом виконання повного перебору.

def held_karp(_distance_matrix):
    n = len(_distance_matrix)
    # Ініціалізація таблиці динамічного програмування
    # Та використання кортежів для представлення наборів міст
    dp = {(frozenset([0, i]), i): (_distance_matrix[0][i], [0, i]) for i in range(1, n)}
            ...
            for next_city in subset:
                ...
                dp[(subset, next_city)] = min(
                    (
                        ...
                        + _distance_matrix[last_city][next_city],
    ...
    result = min(
        (
            dp[(all_cities, last_city)][0] + _distance_matrix[last_city][0],
